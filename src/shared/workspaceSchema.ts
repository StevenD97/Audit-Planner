...
-- Management system maturity assessments (additive; see
-- docs/AUDIT_INTELLIGENCE_PLATFORM_STRATEGY.md's Phase 9 notes).
CREATE TABLE IF NOT EXISTS maturity_assessments (
  id TEXT PRIMARY KEY,
  audit_project_id TEXT NOT NULL,
  data TEXT NOT NULL
);
CREATE INDEX IF NOT EXISTS idx_maturity_assessments_project ON maturity_assessments(audit_project_id);

CREATE TABLE IF NOT EXISTS maturity_dimension_scores (
  id TEXT PRIMARY KEY,
